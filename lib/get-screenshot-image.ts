...
export async function getScreenshotImage({
  ...
  const rasterwise: any = await fetch(
    `https://api.rasterwise.com/v1/get-screenshot?apikey=${process.env.GET_SCREENSHOT_KEY}&url=${encodedUrl}&element=%23preview-parent > div > div&devicefactor=2&forcetr=true`
